...
class TrackFaceUI:
    ...
    def __init__(self):
        ...
        self.__COLOR_YELLOW = (255, 0, 0)  # Red
    ...
    def drawEyesContours(self, frame, leftEye, rightEye):
        ...
        cv2.drawContours(frame, [leftEyeHull], -1, self.__COLOR_YELLOW, 1)
        cv2.drawContours(frame, [rightEyeHull], -1, self.__COLOR_YELLOW, 1)

    # Prints blinks on frame.
    def printBlink(self, frame, total):
        cv2.putText(frame, "Blinks : " + str(total), self.__BLINK_LABEL_LOC,
                    self.__FONT, 1, self.__COLOR_YELLOW, 1)
